    ...
    if (err) {
      ...
      function timedMessages(room, messageArray, ms, logs) {
        messageArray.forEach(function(message, index, array) {
          setTimeout(function() {
            logs = appendMessage(logs, new Date(), message, 'Server');
      ...
      const currentRoom = getCurrentRoom(socket.id, activePlayers, chatters);
      function chatCommands(command) {
        ...
        if (command === '/tutorial') {
          ...
          function tutorialgameUpdates(socket, state, username, room) {
          ...
          tutorialgameUpdates(socket, state, decoded.username, socket.id);
          let context = {
            room: socket.id,
          ...
          function greedyBot(intDifficulty) {
            if (true) {
              ...
              games.get(socket.id).players.set(decoded.username, 1);
              tempState.players = new Map([
                [decoded.username, 0],
                ['greedyBot', 1],
              ]);
              games.set(socket.id, tempState);
              gameUpdates(socket, tempState, decoded.username, socket.id);
            } else {
            }
            ...
            function continuousWalk(coord, occupied, ms, callback) {
              if (occupied.size > 0) {
                ...
                walkingPathfinder(
                  ...
                  // Walk the path
                  takeSteps(val[0], val[1], val.slice(1), callback)
      ...
      if (chatlogs.get(currentRoom)) {
        ...
        io
          .in(currentRoom)
          .emit('chat message', chatMessageEmission(currentRoom, chatlogs));
      }
